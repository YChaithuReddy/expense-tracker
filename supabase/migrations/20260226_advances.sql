...
CREATE POLICY IF NOT EXISTS "Users can view own advances"
    ON advances FOR SELECT
    USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can create own advances"
    ON advances FOR INSERT
    WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own advances"
    ON advances FOR UPDATE
    USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete own advances"
...
CREATE OR REPLACE TRIGGER advances_updated_at
